...
for p0 in seq_p:
    ...
    while count < precesion_control:
        ...
        ###---sp in full maze------###
        final_full, status_full, num2 = func_BFS(Cell(0, 0, 0, dim, None), [dim - 1, dim - 1],
                                                   initial_maze, dim, 0)
        sp_full = []
        len_sp_full = 0
        if status_full == 'solution':
            ...
            ###---run repeated A*--------###
            result, len_trajectory, discovered_maze, processed = fun_repeated_BFS(initial_maze, dim)
            count = count + 1
            sum_traj = sum_traj + len_trajectory
            sum_proc = sum_proc + processed

            ####---sp in discovered maze------###
            final_discoverd, status_disc, num1 = func_BFS(Cell(0, 0, 0, dim, None), [dim - 1, dim - 1],
