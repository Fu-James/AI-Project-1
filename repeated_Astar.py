...
class Repeated_Astar():
    def __init__(self, dim: int, p: float, start: list, goal: list, maze: Gridworld):
        ...
        self._goal = goal
        self._maze = maze
        self._knowledge = Gridworld(self._dim, 0.0) #Create an unblocked gridworld

    def path_walker(self, path: list):
        while path:
            current = path.pop()
            if self._maze.get_cell(current.x, current.y).flag == 1:
                return current.parent
            children = current.getChildren()
            for child in children:
                self._knowledge.update_cell(self._maze.get_cell(child[0], child[1]), child[0], child[1])
