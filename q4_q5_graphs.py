...
def get_data(dim: int, grid_per_pass: int, increment_by: float, option: int):
    ...
    for increment in np.arange(0.01, 1.0, increment_by):
        ...
        for i in range(grid_per_pass):
            ...
            if isSolvable(start, goal, grid, dim):
                solved_count += 1
    ...
    return probability, solvability, duration
...
def isSolvable(start: Cell, goal: list, gridworld: Gridworld, dim: int) -> bool:
    ...
    """
    solution, status = func_Astar(start, goal, gridworld, dim)
    return False if status == "no_solution" else True
    ...
    for dim in dim:
        ...
        for grid_pass in grid_per_pass:
            ...
            for op in range(3):
                ...
                probability, solvability, duration = res
